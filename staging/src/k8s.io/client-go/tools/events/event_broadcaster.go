...
// 调用client-go/kubernetes/typed/events/v1/event.go:events.Create()方法向api server发送event事件
...
// 调用client-go/kubernetes/typed/events/v1/event.go:events.Update()方法向api server发送event事件
...
// 调用client-go/kubernetes/typed/events/v1/event.go:events.Patch()方法向api server发送event事件
...
func newBroadcaster(sink EventSink, sleepDuration time.Duration, eventCache map[eventKey]*eventsv1.Event) EventBroadcaster {
	return &eventBroadcasterImpl{
		// 会初始化一个broadcasterWatcher，用于定义事件处理方式，如上报apiserver等
		Broadcaster:   watch.NewBroadcaster(maxQueuedEvents, watch.DropIfChannelFull),
		// 用于EventBroadcaster和EventRecorder进行事件传输
...
// 对收到的event进行聚合，e.eventCache中已有的更新Series，没有的加入e.eventCache
...
// 将event发送到apiserver。如果count大于1，则进行patch请求（如果返回404则执行create请求），否则执行create请求
...
func (e *eventBroadcasterImpl) StartRecordingToSink(stopCh <-chan struct{}) {
	...
	// 起一个协程获取event信息并将其整理存入eventCache
