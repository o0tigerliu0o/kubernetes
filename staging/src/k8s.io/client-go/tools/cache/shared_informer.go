...
// HandleDeltas具体只干两件事：1、更新informer中的indexer  2、分发资源事件给custom controller(processor.distribute)
// processor.distribute() 通过 processorListener.add() 将资源事件放入processorListener.addCh 完成分发
...
// 监听的事件都是从这个方法传入的，然后写入到addCh管道中
func (p *processorListener) add(notification interface{}) {
	p.addCh <- notification
}
// pop方法在select代码块中会获取addCh管道中的数据，
// 第一个循环的时候notification是nil，所以会将nextCh设置为p.nextCh；第二个循环的时候会将数据写入到nextCh中。
