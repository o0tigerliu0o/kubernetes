...
import (
	...
	schedulerappconfig "kubernetes/cmd/kube-scheduler/app/config"
	"kubernetes/pkg/scheduler"
	kubeschedulerconfig "kubernetes/pkg/scheduler/apis/config"
	kubeschedulerscheme "kubernetes/pkg/scheduler/apis/config/scheme"
...
func (o *Options) Config() (*schedulerappconfig.Config, error) {
	...
	// 1、创建链接api server的client
	client, leaderElectionClient, eventClient, err := createClients(c.ComponentConfig.ClientConnection, o.Master, c.ComponentConfig.LeaderElection.RenewDeadline.Duration)
	if err != nil {
		return nil, err
	}
	// 2、创建EventBroadcaster用于向apiserver发送scheduler自身产生的事件
	...
	c.Client = client
	// 3、创建InformerFactory用于从apiserver获取除pod外其他资源的变更信息
	c.InformerFactory = informers.NewSharedInformerFactory(client, 0)
	// 4、创建PodInformer用于从apiserver获取除pod资源的变更信息
