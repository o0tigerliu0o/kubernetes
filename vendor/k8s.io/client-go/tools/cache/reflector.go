...
// change.
// store参数在controller调用的时候传递的DeltaFIFO类型
...
// 通过backoffManager监视ListAndWatch的运行，异常时可以重新运行
...
/*
 list：获取资源列表，watch：获取资源变更信息，并根据资源变更信息对DeltaFIFO队列进行操作(增删改)
 函数结构一共分为三个块：1、首次获取资源信息(list)  2、起一个常驻协程不断的更新资源信息(list)  3、处理资源变更事件(watch)
 list过程：
 ...
 watch过程：循环调用clientset客户端api与apiServer建立长连接，监控指定资源的变更，如果监控到有资源变更，那么会调用watchHandler处理资源的变更事件
*/
func (r *Reflector) ListAndWatch(stopCh <-chan struct{}) error {
	...
	// 在将首次完整列表同步到Store中后，启动了一个resync的协程。
