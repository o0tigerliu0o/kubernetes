...
// addAllEventHandlers 为 scheduler 提供 eventHandler
// event 的接收者主要有两个，SchedulerCache 和 SchedulingQueue，
// SchedulerCache是为了跟踪集群的资源和已调度 Pod 的状态（addNodeToCache，addPodToCache），
// SchedulingQueue主要是给没有调度的 Pod 入队到 activeQ 中（addPodToSchedulingQueue）
// scheduler 算是在这里完成第一次的过滤。如果是未调度的pod，会经过 event 的过滤器，其中
// assignedPod(t) 会看 pod.Spec.NodeName 判断 pod 是不是已经调度或者 assume 了，
// responsibleForPod(t, sched.Profiles) 会看 pod.Spec.SchedulerName 看一下调度器名字是不是本调度器
